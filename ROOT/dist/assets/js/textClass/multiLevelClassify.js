...
$(".levels_returnresult").html(syntaxHighlight(dataList));

$(".levels_analysis").click(function () {
  var edit_text = filterXSS(
    document.querySelector("#levels_text").value.trim()
  );
  if (edit_text) {
    var editObj = {
      text: edit_text,
    ...
    $.ajax({
      ...
      success: function (res) {
        ...
        if (res.code == 200) {
          ...
          $(".levels_returnresult").html(returnresult);

          if ($("#levels_classify .empty-text").length > 0) {
            $("#levels_classify .empty-text").each(function () {
              $(this).remove();
            });
          }
          if (res.result.length > 0) {
            ...
            $("#levels_list").html(analysis_list);
            ...
            $("#levels_chart").html("");
            new ApexCharts(document.querySelector("#levels_chart"), e).render();
          } else {
            ...
            $(".levels_table").addClass("hide");
            $("#levels_chart").addClass("hide");
            $(".prompt").eq(4).removeClass("hide");
            $(".prompt").eq(5).removeClass("hide");
          }
        }
      },
    });
